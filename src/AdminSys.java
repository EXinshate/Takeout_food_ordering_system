...
public class AdminSys implements DAO<Admin>{
    ...
    public void addDishes(){
        System.out.println("请您输入要添加的菜品：(按照：菜品ID/菜品名/菜品类型/单价/月销量/总数量");
        String str = sc.next();
    ...
    public void selectByDishsID(){
        System.out.println("请输入您的查询的菜品id： ");
        String id = sc.next();
        Dishes dish = d.findById(id);
        if (dish == null){
            System.out.println("没有当前id的菜品");
        }else {
            System.out.println("当前菜品为： " + dish);
            System.out.println("请输入新的菜品单价: ");
            double newprice = sc.nextDouble();
            Dishes t = new Dishes(dish.getdID(), dish.getDname(),
                    dish.getDtype(), dish.getDtime(), newprice,
                    dish.getDsales(), dish.getDistocks());
            d.insert(t);
    ...
    public void changeOrderValue(){
        ...
        if (order == null){
            System.out.println("没有当前id的订单" + order);
        }else {
            System.out.println("已找到当前id订单" + order);
            System.out.println("请输入您要修改的状态： 0.未支付 1.已支付 2.配送中 3.已完成");
            int value = sc.nextInt();
            Order t = new Order(order.getOrderID(), order.getUtime(), order.getDishes(),
                    order.getOrdernum(), order.getuID(), order.getOrderprice(), value);
            o.insert(t);
    ...
    public void addMessage(){
        ...
        Dishes d1 = new Dishes("1", "红烧肉", "肉类", time, 12.5, 20, 30);
        d.insert(d1);
        Dishes d2 = new Dishes("2", "青椒肉丝", "肉类", time, 12, 30, 30);
        d.insert(d2);
        Dishes d3 = new Dishes("3", "回锅肉", "肉类", time, 15, 13, 30);
        d.insert(d3);
        ...
        o.insert(o2);
        Order o3 = new Order("3", localDatetime,d3,5,"1003",40.0,10);
        o.insert(o3);
